...
@tool("list_iam_users",
      description="List IAM users with optional pagination token",
      return_direct=False)
def list_iam_users(token="", max_items=100):
    """List IAM users with optional pagination token."""
    client = session.get_client("iam")
    next_token = token or None
    return iam.list_users(client, marker=next_token, max_items=max_items)
